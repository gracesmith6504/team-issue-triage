function buildKPIs() {
  ...
  var kpis = [
    {value: d.summary.triage_needed, label: "Issues Needing Triage", hint: "Issues with the state:triage-needed label on GitHub", sub: d.summary.total_open + " total open issues", color: "var(--urgency-high)", spark: d.sparklines.triage, sparkColor: "#e16f24"}
  ];

  if (d.pr_health) {
    kpis.push({value: d.pr_health.awaiting_review, label: "PRs Waiting for Review", sub: d.pr_health.stale_14d + " stale (14d+)", color: "var(--status-waiting)", spark: d.sparklines.prs, sparkColor: "#d4a015", target: "pr-health", tileFilter: "awaiting", tileAccent: "var(--status-waiting)"});
  }
  if (d.vouch_status) {
